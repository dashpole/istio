	...
	dashboards = []struct {
		...
		},
		{
			...
			[]string{
				"galley_",
				"istio_tcp_",
				`max(pilot_k8s_cfg_events{type="WorkloadEntry|ServiceEntry|PeerAuthentication|RequestAuthentication|AuthorizationPolicy", event="add"}) - (max(pilot_k8s_cfg_events{type="WorkloadEntry|ServiceEntry|PeerAuthentication|RequestAuthentication|AuthorizationPolicy", event="delete"}) or max(up * 0))`,
			},
		},
		{
			"istio-services-grafana-dashboards",
